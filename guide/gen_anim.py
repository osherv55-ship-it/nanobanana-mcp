...
def s09(u):
    ov = ""
    for d, (a, b) in zip(["M286,250 C298,240 308,232 316,222", "M134,250 C122,240 112,232 104,222"],
                         seq(2, .05, .5)):
        ov += A(stroke(d, Z["glow"], 10, .6), "draw", a, b)
    # a dashed keep-out marker over the tear trough: this band gets no highlight
    ov += clipped(u, A(f'<path d="M228,246 C244,262 262,266 278,262" fill="none"'
                       f' stroke="{Z["no"]}" stroke-width="2.4" stroke-opacity=".8"'
                       f' stroke-dasharray="6 5"/>', "fade", .54, .68))
    ov += clipped(u, A(f'<path d="M192,246 C176,262 158,266 142,262" fill="none"'
                       f' stroke="{Z["no"]}" stroke-width="2.4" stroke-opacity=".8"'
                       f' stroke-dasharray="6 5"/>', "fade", .54, .68))
    pts = [(210,361),(210,420)]
    for (x, y), (a, b) in zip(pts, seq(2, .68, .92, .08)):
        ov += A(f'<g><circle cx="{x}" cy="{y}" r="13" fill="{Z["glow"]}" fill-opacity=".3"/>'
                f'<circle cx="{x}" cy="{y}" r="6" fill="{Z["glow"]}" fill-opacity=".85"/></g>',
                "pop", a, b, cx=x, cy=y)
    return scene(u, ov, label="הארה — הרחק מאזור העין"), "מעל עצם הלחי · לא ליד העין", 8
...
TITLES = {
 ...
 9: "הארה — הרחק מהעין", 10: "שפתיים — הגדרה והרמה", 11: "קיבוע — ולחיצה",
}
