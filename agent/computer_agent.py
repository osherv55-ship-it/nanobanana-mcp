...
def main():
    ...
    base = ("את סוכן שמפעיל מחשב Windows כדי לערוך וידאו ב-CapCut. "
              "🚫 קריטי — אסור לחלוטין: לעולם אל תסגרי או תלחצי על כפתור ה-X / כפתור הסגירה של שום חלון, "
              "ובמיוחד לא של חלון PowerShell/Terminal — חלון ה-PowerShell הוא זה שמריץ אותך, וסגירתו תהרוג אותך מיד. "
              "אל תלחצי לעולם בפינה הימנית-עליונה של חלון אם את חושדת שזה כפתור סגירה. "
              "🚫 אל תזיזי, תמזערי, או תזיזי חלונות — רק עברי ביניהם. "
              "כדי להביא את CapCut לפוקוס: לחצי Alt+Tab (action 'key' עם 'alt+Tab'); אם CapCut לא קופץ, חזרי על Alt+Tab עד שתראי את ממשק CapCut (טיימליין/תצוגה מקדימה/Templates). "
              "אל תבזבזי צעדים בסריקת שורת המשימות עם העכבר — Alt+Tab מהיר ובטוח יותר. "
              "הנח ש-CapCut כבר פתוח אי-שם; המשימה שלך היא להביא אותו לקדמת המסך עם Alt+Tab ואז לעבוד בו. "
              "אם את רואה אפליקציה אחרת (Chrome, הגדרות, וואטסאפ) — אל תתעסקי איתה ואל תסגרי אותה; פשוט Alt+Tab ל-CapCut. "
              "אל תפתחי תפריט התחל, אל תשתמשי בחיפוש של Windows, ואל תיבהלי מפופ-אפים פנימיים של CapCut — התעלמי והמשיכי. "
