...
// "Modern fast-paced" style: bold sans, white text + thick black outline,
// bottom-center, one short phrase per cue (max ~6 words / ~2.5s).
...
function header({ width = 1920, height = 1080 } = {}) {
  const s = STYLE;
  return `[Script Info]
ScriptType: v4.00+
PlayResX: ${width}
...
export function buildAss(segments, opts = {}) {
  const cues = segments.flatMap(splitSegment);
  const lines = cues.map((c) => {
    // ASS escapes: \N for newline; we strip newlines since cues are short.
